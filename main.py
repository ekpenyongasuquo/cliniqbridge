...
async def root_post(request: Request):
    """Handle POST to root — used by Prompt Opinion during MCP registration."""
    return {
        "name": "CliniqBridge",
        "version": "1.0.0",
        "description": "MCP server for FHIR patient summary and history retrieval",
...
async def post_patient_summary(
    ...
    pid = body.parameters.get("patient_id") or body.patient_id or x_patient_id
    return await get_patient_summary(pid, x_patient_id, x_fhir_base_url, x_fhir_token)
...
async def post_conditions(
    ...
    pid = body.parameters.get("patient_id") or body.patient_id or x_patient_id
    return await get_conditions(pid, x_patient_id, x_fhir_base_url, x_fhir_token)


@app.post("/tools/get_medications")
async def post_medications(
    body: ToolCallRequest,
    x_patient_id: Optional[str] = Header(default=None),
    x_fhir_base_url: Optional[str] = Header(default=None),
    x_fhir_token: Optional[str] = Header(default=None),
    ...
    pid = body.parameters.get("patient_id") or body.patient_id or x_patient_id
    return await get_medications(pid, x_patient_id, x_fhir_base_url, x_fhir_token)


@app.post("/tools/get_encounters")
async def post_encounters(
    body: ToolCallRequest,
    x_patient_id: Optional[str] = Header(default=None),
    x_fhir_base_url: Optional[str] = Header(default=None),
    x_fhir_token: Optional[str] = Header(default=None),
    ...
    pid = body.parameters.get("patient_id") or body.patient_id or x_patient_id
    return await get_encounters(pid, x_patient_id, x_fhir_base_url, x_fhir_token)
